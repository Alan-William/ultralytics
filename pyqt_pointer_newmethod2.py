...
class ImageDetection(QWidget):
    def __init__(self):
        ...
        self.model_path = os.getenv('YOLO_MODEL_PATH', os.path.join(self.flie_path, 'runs', 'sets', '3pointer_newmethod2', 'weights', 'best.pt'))    # 权重文件路径
        self.txt_path = os.path.join(self.flie_path, 'Result_pointer_newmethod2.txt')  # 读数保存路径
        self.imagepath = self.before_imagepath
        self.image = None
        self.outputPath = ('outputs2/')
        self.imageName = None
        self.circleimg = None
    ...
    def Readvalue(self):
        ...
        self.result = self.yolo(source=self.before_imagepath, save=True, conf=0.7, device='cpu')
