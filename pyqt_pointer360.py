...
class ImageDetection(QWidget):
    def __init__(self):
        ...
        self.model_path = os.getenv('YOLO_MODEL_PATH', os.path.join(self.flie_path, 'runs', 'sets', '4pointer360', 'weights', 'best.pt'))    # 权重文件路径
        self.txt_path = os.path.join(self.flie_path, 'Result_pointer360.txt')  # 读数保存路径
    ...
    def Readvalue(self):
        ...
        self.result = self.yolo(source=self.before_imagepath, save=True, conf=0.6, device='cpu')

        data = self.result[0].boxes.data.cpu().numpy()
        self.zeroPoint = [float((data[0][0] + data[0][2])/2), float((data[0][1] + data[0][3])/2)]
        ...
        # self.imageLabel1.setPixmap(pixmap.scaled(self.imageLabel1.size(), Qt.KeepAspectRatio))
        scaled_pixmap = pixmap.scaled(600, 600)  # 将图像缩放
