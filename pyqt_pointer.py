...
class ImageDetection(QWidget):
    def __init__(self):
        ...
        self.model_path = os.getenv('YOLO_MODEL_PATH', os.path.join(self.flie_path, 'runs', 'sets', '3pointer', 'weights', 'best.pt'))    # 权重文件路径
        self.txt_path = os.path.join(self.flie_path, 'Result_pointer.txt')  # 读数保存路径
